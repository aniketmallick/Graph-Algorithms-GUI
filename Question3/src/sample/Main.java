...
//        this.importGraph("input.txt");
    ...
    public void importGraph(String stri) throws FileException {
        String[] str;
        try
        {   BufferedReader br = new BufferedReader(new FileReader("C:\\Users\\Asus\\IdeaProjects\\Question3\\src\\sample\\"+stri));
    ...
    public void exportGraph(String stri) throws FileException {
        ...
        try(BufferedWriter br = new BufferedWriter(new FileWriter("C:\\Users\\Asus\\IdeaProjects\\Question3\\src\\sample\\"+stri)))
        {
            String str="";
...
public class Main extends Application {
    ...
    String path="C:\\Users\\Asus\\IdeaProjects\\Question3\\src\\sample\\";
    String nullable="";
...
//                newWindow.setHeight(500);
                ...
                try {
                    ...
                    for(int k=0;k<(dijkedgeList.size()-1);k++){
                            ...
                            t.setStroke(Color.DARKRED);
                            t.setStrokeWidth(4);
                    }
                    HBox h1 = new HBox();String tmp="";int ind =-1;
                    for(Integer it : dijkedgeList){
                        ind++;
                        tmp += (vertexList.get(it).getName());
                        if(ind<(dijkedgeList.size()-1)){
                            tmp += "  - >  ";
                        }
                    }
                    Label temp = new Label("Dijkstra's Path Cost is : " + (int)cost+ "\nDijkstra's Path is : "+tmp);
...
//                    alert.setContentText(nullable);
                                    ...
                                    try {
                                        ...
                                        alert.setAlertType(Alert.AlertType.ERROR);
                                        alert.setHeaderText("Please provide weight in double value");
                                        alert.show();
                                    }
                                    tid.getEditor().clear();
                                    indexOfLine=-1;
